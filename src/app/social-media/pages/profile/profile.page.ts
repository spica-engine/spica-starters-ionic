...
export class ProfilePage {
  ...
  deletedPost(event) {
    this.loading = true;
    this._postService.deletePost(event).then((_) => {
  ...
  subscribeToTabsEvent() {
    this.tabsSubscription = this._tabService
      .listenTabClick()
      .subscribe((tab_info: string) => {
        let post_id = tab_info.split('profile_')[1];
        if (post_id && this.user && this.user._id == this.me._id) {
          this._postService
