...
export class ChatService {
  ...
  async initialize() {
    ...
    // this.dataService.resources.chat_group
    //   .getAllRealtime({
    //     filter: {
    ...
    //   })
    //   .pipe(
    //     map((chats: ChatGroups[]) => {
    //       chats = chats.map((chat: ChatGroups) => {
    //         let userObject = chat.last_active.filter(
    //           (user) => user.user == this.me._id || user.user._id == this.me._id
    //         )[0];
    ...
    //       })
    //     ),
    //     switchMap((chats: ChatGroups[]) => {
    //       let nonCachedUsers = [];
    //       chats.forEach((chat: ChatGroups) => {
    ...
    //           ? this.dataService.resources.user.getAll({
    //               queryParams: {
    //                 filter: {
    ...
    //   )
    //   .subscribe((data) => {
    //     this.chatGroups = data.map((chat: ChatGroups) => {
    //       if (!chat.is_group) {
    //         chat.name =
          ...
          if (data[0])
            for (let item of data[0].last_active) {
              if (item.user == this.me._id) item.user = this.me as any;
                ...
                item.user = this.cachedUsers.find((cu) => cu._id == item.user) as any;
              } else {
                item.user = await user
                  .get(item.user as string)
  ...
  createChat(chatGroup: Chat) {
    // chatGroup.managers = [this.me._id as User];
    // return this.dataService.resources.chat_group.post(chatGroup);
  }
  async isChatExists(users: User[]) {
    ...
    //   )[0]._id;
      ...
      // let deletedChat = await this.getChatByUserWithMe(other_user).toPromise();
